...
def type_based_summary_node(state: InterviewState) -> InterviewState:
    ...
    try:
        # Load FAISS index using the SAME embeddings model it was created with
        vector_db = FAISS.load_local(state.vector_db_path, GLOBAL_EMBEDDINGS_MODEL, allow_dangerous_deserialization=True)
        # For full summaries, we might want to retrieve *all* relevant chunks, not just top K
        # A simple retriever will give top-k based on query similarity.
        # For a full summary, we want all the "representative" chunks identified earlier,
        # or, if the context permits, all chunks from the DB.
        
        # Option 1: Use the representative chunks already identified
        # This is generally more robust for full summaries as it respects your pre-calculated importance
        if state.representative_chunks_for_summary:
            # Convert representative chunks to LangChain Document objects
            # create_stuff_documents_chain expects Document objects
            from langchain_core.documents import Document
            relevant_docs = [
                Document(page_content=chunk["text"], metadata=chunk["metadata"])
                for chunk in state.representative_chunks_for_summary
            ]
            logger.info(f"Using {len(relevant_docs)} representative chunks for summary context.")
            # For this option, we don't need `create_retrieval_chain` for the input
            # since we already have the documents. We'll directly pass them to the combine_docs_chain.
            
        else:
            # Fallback: If for some reason representative_chunks_for_summary is empty,
            # retrieve from vector DB using a general query or get all documents if possible.
            # Retrieving top-k for a summary is often a trade-off.
            # You could also try to retrieve all documents if the DB is small enough.
            logger.warning("No representative chunks found, falling back to general retrieval from vector DB.")
            retriever = vector_db.as_retriever(search_kwargs={"k": 50}) # Increased k for better coverage
            # A general input might be used to get "overall" relevant documents
            relevant_docs = retriever.invoke("summarize the entire podcast episode") # Provide a meaningful query
            logger.info(f"Retrieved {len(relevant_docs)} documents from FAISS for summary context.")
...
**(Continue this format for all questions in the podcast)**
    """)
            ...
            # Create separate chains for summary and Q&A
            doc_chain_summary = create_stuff_documents_chain(llm=llm, prompt=prompt_summary)
            doc_chain_qa = create_stuff_documents_chain(llm=llm, prompt=prompt_qa)

            # Invoke both, passing the relevant_docs directly to combine_docs_chain's invoke
            # because we already retrieved/selected them.
            # We don't need create_retrieval_chain if we're directly stuffing pre-selected docs.
            response_summary = doc_chain_summary.invoke({"input": relevant_docs, "context": relevant_docs})
            response_qa = doc_chain_qa.invoke({"input": relevant_docs, "context": relevant_docs})
